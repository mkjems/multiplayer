...
function handleGameState(msg) {
    ...
    for (const b of msg.bullets) {
        ...
        if (b.bounces > prevBounces) {
            ...
            if (trail.length > CONSTANTS.TRAIL_MAX_POSITIONS)
                trail.shift();
    ...
    gameState.cacti = msg.cacti;
    countEl.textContent = `${gameState.players.length} player${gameState.players.length !== 1 ? "s" : ""}`;
    const me = gameState.getLocalPlayer();
    if (me)
        gameState.localFacing = me.facing;
}
